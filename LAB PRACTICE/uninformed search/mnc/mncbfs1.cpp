...
void printOpenClosedLists(queue<vector<State>> &open, unordered_set<int> &closed) {
    cout << "Open List:" << endl;
    queue<vector<State>> temp = open;
    while (!temp.empty()) {
        vector<State> path = temp.front();
        temp.pop();
...
vector<State> bfs(State &initial, State &goal) {
    if (!isValid(initial)) {
        return {};
    }

    unordered_set<int> visited;
    queue<vector<State>> open;
    open.push({initial});

    while (!open.empty()) {
        vector<State> path = open.front();
        open.pop();
