// without struct - working after making changes in queue
#include <bits/stdc++.h>
#include <vector>
#include <queue>
#include <set>
...
void solve(vector<vector<int>> &board, vector<vector<int>> &goal, int x, int y, int depth)
{
    ...
    priority_queue<pair<int, pair<int, vector<vector<int>>>>, vector<pair<int, pair<int, vector<vector<int>>>>>, greater<>> pq;
    ...
    while (!pq.empty())
    {
        ...
        vector<vector<int>> current = currentPair.second.second;
        g = currentPair.second.first;
        pq.pop();
        statesExplored++;
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            if (newX >= 0 && newX < current.size() && newY >= 0 && newY < current.size())
            {
                ...
                // pq.push({(h + g), current});
                pq.push({(h + g + 1), {g + 1, current}});
                cout << "g : " << g << endl;
                cout << "h : " << h << endl;
                cout << "cost fun : " << h + g << endl;
                printBoard(current);
                count++;
